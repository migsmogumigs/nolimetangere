...
onClipEvent (load) {
	...
	oObj 				= new Array ( _parent.frame , _parent.page , thumb , hitclip.invis, _parent.frameMask);
	sObj.len 			= gObj.length;
	
	// setting not changeable properties
	sObj.frameMask 			= oObj[4]._height //added by iabuabu nov 17
	...
	function setOnOff () {
		if ( sObj.pageHO <> sObj.pageH ) {
			trace("set on off: "+sObj.pageH + " : " +sObj.frameMask);
			//if ( sObj.pageH >= sObj.frameH ) { //removed by iabuabu nov 17
			if ( (sObj.pageH-162) >= sObj.frameMask ) { //added by iabuabu nov 17
				var count		= 0;
				...
				oObj[2]._y 		= 0;
				bulbObj[0]._y 	= 0;
			//} else if ( sObj.pageH < sObj.frameH ) { //removed by iabuabu nov 17
			} else if ( (sObj.pageH-162) < sObj.frameMask ) { //added by iabuabu nov 17
				var count 		= 0;
				while ( count < sObj.len ) {
	...
	function gotoScroll (num:Number) {
		trace("gotoscroll: "+num);
		if(num==undefined) oObj[2]._y = (oObj[3]._y/100) * sObj.scrollA;
		else oObj[2]._y = num;
		stopDrag ();
	}
	function moveUpDown(){
